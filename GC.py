"""gc.py — Pipeline de análise cromatográfica GC/HPLC.

Leitura de CDF → subtração de baseline → detecção de picos → integração
adaptativa (EMG, Drop-Line, Tangent Skim, Deconvolução) → métricas USP/EP
→ relatório PDF (GCReport) → visualização interativa (GCVisualizer).

Todos os tempos em segundos. Thread-safe por instância (não compartilhe GCAnalyzer).
"""
...
# ══════════════════════════════════════════════════════════════════════════════
class GCReport:
    """Gera relatório analítico em PDF (ReportLab) para uma ou mais corridas GC.

    Seções: capa, parâmetros do método, cromatograma+tabela por corrida,
    comparação entre corridas (≥2) e apêndice normativo (USP/EP).
    """
    ...
    class _Ruled(HRFlowable):
        """Linha horizontal decorativa com paleta e defaults do GCReport."""
    ...
    @dataclass
    class _RunData:
        """Dados de uma corrida: arrays brutos + DataFrame de picos + label."""
    ...
    def __init__(
        ...
        """Inicializa o relatório com metadados (título, analista, instrumento, etc.)."""
...
class GCAnalyzerError(Exception):
    """Exceção base do pipeline. Carrega `context` dict com detalhes do erro."""
...
@dataclass
class RunResult:
    """Resultado de uma corrida individual do process_batch (status OK/FAILED + DataFrame de picos)."""
...
class ProcessingMethod:
    """Parâmetros do método analítico. Serializável em JSON para rastreabilidade."""
    ...
    t_start_integration: float = (
        0.0  # t_start_integration : tempo de início da integração em segundos. Pontos de tempo anteriores a este valor são excluídos antes da detecção de picos. Útil para ignorar o pico de injeção (solvent front) e a instabilidade inicial do detector. 0.0 -> sem corte (integra desde o início do cromatograma).
    )
    integration_inhibit_windows: list = field(
        default_factory=list
    )  # integration_inhibit_windows : janelas de inibição da integração. Lista de tuplas (t_start, t_end) em segundos.  Qualquer pico cujo ápice caia dentro de uma dessas janelas é silenciosamente descartado durante a detecção. Use para mascarar perturbações conhecidas como trocas de válvula, pulsos de pressão ou artefatos de gradient delay. Exemplo: [(120.0, 125.0), (300.0, 302.5)]. [] -> nenhuma janela de inibição ativa.
    ...
    dead_time_s: float = (
        0.0  # dead_time_s : tempo morto (t₀) da coluna em segundos. Necessário para calcular k' = (tR - t₀) / t₀. 0.0 → recurso desativado (k' e α ficam NaN). Pode ser medido pela injeção de metano (FID) ou determinado por geometria da coluna: t₀ ≈ L / (u × (1 + k)) onde u é fluxo linear.
    )
...
@dataclass
class AuditEvent:
    """Registro imutável de um evento do pipeline (timestamp, nível, módulo, mensagem)."""
...
class AuditLogger:
    """Audit trail estruturado: registra e exporta eventos INFO/WARN/ERROR/DECISION do pipeline."""
    ...
    def to_dataframe(self):
        ...
        for e in self._events:
            d = e.to_dict()
            d["context"] = json.dumps(d["context"], ensure_ascii=False, cls=_NumpyEncoder)
            rows.append(d)
...
class GCAnalyzer:
    """Motor principal do pipeline: leitura CDF → baseline → detecção → integração → métricas."""
    ...
    def read_cdf(self, cdf_filepath):
        """Lê arquivo ANDI CDF e retorna (retention_time, intensity) como arrays numpy."""
        try:
    ...
    def estimate_noise_level(self, intensity):
        """Estima ruído global pelo método MAD (Median Absolute Deviation). Retorna sigma."""
    ...
    def estimate_local_snr(self, rt, intensity, peak_idx, left_bound, right_bound, width_factor=3.0):
        """Calcula SNR local de um pico usando regiões adjacentes. Retorna (snr, baseline_local, noise)."""
    ...
    def integrate_trapezoid_segment(self, rt, intensity, left, right):
        """Integração trapezoidal de um segmento acima da baseline virtual. Retorna (area, x, y_above, baseline)."""
        x_seg = rt[left:right]
    ...
    def _slope_to_slope_bounds(
        ...
        """Determina limites de integração por sensibilidade de inclinação (slope sensitivity),
        análogo ao critério do Agilent OpenLab/Chromeleon. Retorna (left, right) como índices."""
        # ── Lado esquerdo ────────────────────────────────────────────────────
    ...
    def emg(x, A, mu, sigma, tau):
        """Modelo Exponentially Modified Gaussian (EMG) — pico com tailing."""
        if sigma <= 0 or tau <= 0:
    ...
    def multi_emg(x, *params):
        """Soma de N modelos EMG (params em blocos de 4: A, mu, sigma, tau)."""
    ...
    def gaussian(x, A, mu, sigma):
        """Gaussiana simples — usado como ajuste de referência e fallback."""
        return A * np.exp(-((x - mu) ** 2) / (2 * sigma**2))

    @staticmethod
    def multi_gaussian(x, *params):
        """Soma de N gaussianas (params em blocos de 3: A, mu, sigma)."""
        y = np.zeros_like(x)
    ...
    def _emg_moments_p0(x: np.ndarray, y: np.ndarray, peak_rt: float) -> tuple[float, float, float, float]:
        """Estima p0=[A, mu, sigma, tau] para o fit EMG via momentos estatísticos do segmento."""
        w = np.maximum(y, 0.0)
        w_sum = w.sum()
    ...
    def fit_emg_peak(self, rt, intensity, peak_idx, left, right):
        """Ajusta modelo EMG ao segmento [left:right]. Retorna dict com parâmetros ou None se janela pequena demais."""
        x = rt[left:right]
    ...
    def find_valley(self, intensity, peak1_idx, peak2_idx):
        """Retorna o índice do mínimo (vale) entre dois picos."""
    ...
    def calculate_valley_metrics(self, rt, intensity, peak1_idx, peak2_idx):
        """Calcula %vale e razão de altura entre dois picos. Retorna (valley_idx, valley_pct, height_ratio)."""
    ...
    def classify_overlap(self, rt, intensity, peak1_idx, peak2_idx):
        """Classifica o tipo de sobreposição entre dois picos: INDEPENDENT, DROP_LINE, TANGENT_SKIM ou DECONVOLUTION."""
    ...
    def integrate_dropline(self, rt, intensity, peak1_idx, peak2_idx, valley_idx, left_base, right_base):
        """Integra dois picos sobrepostos pelo método Drop-Line (separação no vale). Retorna lista de dicts."""
        self.audit.info(
            "Integration",
    ...
    def integrate_tangent_skim(self, rt, intensity, parent_idx, rider_idx, valley_idx, left_base_parent, right_base_rider):
        """Integra par de picos pai/rider pelo método Tangent Skim. Retorna lista de dicts."""
        self.audit.info(
            "Integration",
    ...
    def peak_width_at_fraction(self, rt, intensity, peak_idx, fraction: float):
        """Calcula largura do pico em uma fração da altura. Retorna (width, left_x, right_x, front, tail)."""
    ...
    def asymmetry_factor(self, rt, intensity, peak_idx) -> float:
        """Asymmetry Factor EP (dt/df a 10% da altura)."""
    ...
    def theoretical_plates(self, rt, intensity, peak_idx) -> float:
        """N (pratos teóricos) pela fórmula EP: 5.54 × (tR/W½)²."""
    ...
    def percent_rsd(areas: list | np.ndarray) -> dict:
        """Calcula média, std e RSD% de uma lista de áreas. Retorna dict com status PASS/FAIL (≤2%)."""
    ...
    def remove_solvent_peak(self, df, protected_rt: Optional[float] = None):
        """Remove picos de solvente (RT curto ou área >> mediana). protected_rt preserva o IS."""
        if df.empty:
            return df
    ...
    def _run_forced_integrations(self, rt: np.ndarray, intensity: np.ndarray) -> list[dict]:
        """Integra regiões de `force_integration_windows` ignorando a detecção automática.
        Retorna lista de dicts com integration_method='FORCED'."""
    ...
    def find_internal_standard(self, df: pd.DataFrame) -> pd.Series:
        """Localiza o pico do Padrão Interno (IS) no DataFrame pelo RT configurado ou maior área."""
        if df.empty:
    ...
    def align_runs(self, runs: list[tuple[str, pd.DataFrame]], auto_is: bool = True) -> tuple[pd.DataFrame, pd.DataFrame]:
        """Alinha múltiplas corridas por RRT e agrupa picos em bins. Retorna (df_long, df_stats)."""
        if not runs:
    ...
    def _handle_batch_error(self, results, run_id, cdf_path, exc, file_idx, n_total, summary):
        """Registra erro de corrida no audit trail e adiciona RunResult(FAILED) à lista."""
    ...
    def compute_extended_metrics(
        ...
        """Adiciona ao DataFrame (pós compute_usp_metrics): Rs_usp, Rs_ep, k_prime,
        alpha, shape_quality_score (SQS), rsd_pct_area e CQI."""
        m = self._m
    ...
    def compute_global_metrics(
        ...
        """Calcula métricas globais da corrida: drift de baseline, SNR global,
        área integrada, cobertura, OQS e metadados de duração/amostragem."""
    ...
    def compare_chromatograms(
        ...
        *,
        ...
        """Fingerprinting ponto-a-ponto de dois cromatogramas.
        Retorna dict com pearson_r, rmse, cosine_similarity, mae, max_abs_diff e verdict."""
    ...
    def export_audit(self, path_json: Optional[str] = None, path_csv: Optional[str] = None) -> dict:
        """Exporta audit trail para JSON e/ou CSV. Retorna dict com run_id, method, summary e events."""
        if path_json:
    ...
    def plot_results(self, rt, raw, baseline, corrected, results_df):
        """Gera gráfico interativo Plotly com sinal bruto, baseline, picos e curvas EMG ajustadas."""
        fig = go.Figure()
...
class GCVisualizer:
    """
    Visualizador interativo de cromatogramas GC.

    Responsabilidade única: renderização de gráficos Plotly interativos
    a partir de dados já processados pelo GCAnalyzer.

    Parâmetros
    ----------
    default_output_dir : str | Path
        Diretório padrão para salvar HTMLs quando não especificado em cada
        chamada de plot. Se None, usa o diretório de trabalho atual.
    show_on_plot : bool
        Se True, abre o navegador ao chamar qualquer método plot_*.
        Útil para uso interativo; defina False em pipelines automatizados.
    line_width : float
        Largura das linhas de cromatograma.
    marker_size : int
        Tamanho dos marcadores de pico.
    """

    # ──────────────────────────────────────────────────────────────────────────────
    # Paleta qualitativa robusta (distinguível mesmo em impressão)
    # ──────────────────────────────────────────────────────────────────────────────
    _PALETTE: list[str] = [
        "#1f77b4",  # azul
        "#d62728",  # vermelho
    ...
    # ──────────────────────────────────────────────────────────────────────────────
    # Estrutura interna de dados de uma corrida
    # ──────────────────────────────────────────────────────────────────────────────
    @dataclass
    class _VisRun:
        """Dados de uma única corrida GC para visualização."""

        label: str
        rt: np.ndarray  # tempo de retenção em segundos
        signal: np.ndarray  # intensidade (pós-baseline)
        results_df: pd.DataFrame  # DataFrame de resultados do GCAnalyzer
        color: str = "#1f77b4"

        # ── Propriedades computadas ────────────────────────────────────────────

        @property
        def rt_min(self) -> np.ndarray:
            """Tempo de retenção em minutos."""
            return self.rt / 60.0

        @property
        def peak_rows(self) -> list[dict]:
            """Linhas do results_df convertidas para lista de dicts (cada pico)."""
            if self.results_df is None or self.results_df.empty:
                return []
            return self.results_df.to_dict(orient="records")

        @property
        def n_peaks(self) -> int:
            return len(self.peak_rows)

    # ──────────────────────────────────────────────────────────────────────────────
    # Métodos Estáticos Auxiliares (Refatorados)
    # ──────────────────────────────────────────────────────────────────────────────

    @staticmethod
    def _fmt(val, decimals: int = 2, fallback: str = "—") -> str:
        """Formata um valor numérico com fallback para NaN/None."""
        try:
            f = float(val)
    ...
    @staticmethod
    def _peak_hover_text(row: dict, peak_num: int) -> str:
        """Monta o texto de hover para um marcador de pico."""
        # Alias local para brevidade
        fmt = GCVisualizer._fmt
        is_fin = GCVisualizer._is_finite

        rt_s = row.get("marker_rt", row.get("rt", np.nan))
        rt_m = rt_s / 60.0 if np.isfinite(float(rt_s)) else np.nan
        area = row.get("area", np.nan)
        snr = row.get("snr", np.nan)
        area_p = row.get("area_pct", np.nan)
        method = row.get("integration_method", "")
        n_ep = row.get("N_plates_ep", np.nan)
        rs = row.get("Rs_ep", row.get("Rs_usp", np.nan))
        tf = row.get("tailing_factor_usp", np.nan)
        w_half = row.get("w_half_s", row.get("w_half", np.nan))

        lines = [
            f"<b>Pico #{peak_num}</b>",
            f"RT: {fmt(rt_m, 3)} min  ({fmt(rt_s, 1)} s)",
            f"Área: {fmt(area, 0)}  ({fmt(area_p, 2)} %)",
            f"SNR: {fmt(snr, 1)}",
        ]
        if is_fin(w_half):
            lines.append(f"W½: {fmt(w_half, 2)} s")
        if is_fin(n_ep):
            lines.append(f"N (EP): {int(float(n_ep))}")
        if is_fin(rs):
            lines.append(f"Rs: {fmt(rs, 2)}")
        if is_fin(tf):
            lines.append(f"TF: {fmt(tf, 3)}")
        if method:
            lines.append(f"Método: {method}")
        return "<br>".join(lines)

    @staticmethod
    def _resolve_output(output_dir: str | Path, filename: str) -> Path:
        """Garante que o diretório existe e retorna o Path completo."""
    ...
    @staticmethod
    def _default_html_name(prefix: str, labels: list[str]) -> str:
        ts = datetime.now().strftime("%d%m%Y_%H%M%S")
        sample_part = " + ".join(labels[:3])
        if len(labels) > 3:
            sample_part += f" (+{len(labels) - 3})"
        return f"{prefix} — {sample_part} — {ts}.html"

    @staticmethod
    def _apply_common_layout(fig: go.Figure, title: str) -> None:
        """Aplica layout padrão a qualquer figura GCVisualizer."""
        fig.update_layout(
            title=dict(text=title, font=dict(size=14, color="#1B2A4A"), x=0.5, xanchor="center"),
            hovermode="closest",
            dragmode="zoom",
            template="plotly_white",
            height=620,
    ...
    def __init__(
        self,
        default_output_dir: str | Path = ".",
        show_on_plot: bool = True,
        line_width: float = 1.5,
        marker_size: int = 10,
    ):
        self.default_output_dir = Path(default_output_dir).expanduser().resolve()
        self.show_on_plot = show_on_plot
        self.line_width = line_width
        self.marker_size = marker_size

        self._runs: list[GCVisualizer._VisRun] = []
    ...
    def add_run(
        ...
        """Adiciona corrida ao visualizador. color=None seleciona da paleta automaticamente. Retorna self."""
        if color is None:
            color = self._PALETTE[len(self._runs) % len(self._PALETTE)]

        rt_arr = np.asarray(rt, dtype=float)
        sig_arr = np.asarray(signal, dtype=float)

        if rt_arr.shape != sig_arr.shape:
            raise ValueError(f"[GCVisualizer] rt e signal têm formas diferentes: " f"{rt_arr.shape} vs {sig_arr.shape}")
    ...
    def add_runs_from_batch(
        self,
        batch_results: list,  # list[RunResult] de GCAnalyzer.process_batch
        analyzer,  # instância de GCAnalyzer (para read_cdf / remove_baseline)
        labels: Optional[dict] = None,  # {run_id: label_customizado}
    ) -> "GCVisualizer":
        """Popula o visualizador a partir de process_batch(), re-lendo os sinais CDF."""
        for r in batch_results:
            if not r.ok:
                warnings.warn(
                    f"[GCVisualizer] Corrida {r.run_id!r} com status FAILED — ignorada.",
                    stacklevel=2,
                )
                continue
            try:
                rt, raw = analyzer.read_cdf(r.cdf_path)
                corrected, _ = analyzer.remove_baseline(rt, raw)
            except Exception as exc:
                warnings.warn(
                    f"[GCVisualizer] Falha ao re-ler/re-processar {r.cdf_path!r}: {exc} — ignorada.",
                    stacklevel=2,
                )
                continue

            lbl = (labels or {}).get(r.run_id, r.run_id)
            self.add_run(label=lbl, rt=rt, signal=corrected, results_df=r.results_df)

        return self

    def clear(self) -> "GCVisualizer":
        """Remove todas as corridas adicionadas."""
        self._runs.clear()
        return self

    def runs(self) -> list[str]:
        """Retorna os labels das corridas adicionadas."""
        return [r.label for r in self._runs]
    ...
    def plot_single(
        ...
        *,
        ...
        """Plota cromatograma individual com marcadores de pico. Salva HTML e retorna Figure."""
        run = self._get_run(label)
        fig = go.Figure()
        self._add_chromatogram_trace(fig, run, show_in_legend=True)
        self._add_peak_markers(fig, run, show_in_legend=True)

        title_str = title or f"Cromatograma — {run.label}"
        self._apply_common_layout(fig, title_str)
        fig.update_layout(yaxis=dict(title="Intensidade (u.a.)", showgrid=True, gridcolor="#ececec", zeroline=False))

        _fname = filename or self._default_html_name("single", [run.label])
        self._save_and_show(fig, output_dir, _fname, show)
        return fig
    ...
    def plot_overlay(
        ...
        *,
        ...
        """Sobrepõe múltiplos cromatogramas em um gráfico. normalize=True normaliza pelo máximo."""
        runs = self._select_runs(labels)
        if not runs:
            raise ValueError("[GCVisualizer] Nenhuma corrida selecionada para plot_overlay.")
        ...
        for run in runs:
            ...
            if normalize:
                ...
                sig_plot = sig / max_v if max_v > 0 else sig
                # Cria uma instância temporária para plotagem
                _run_plot = self._VisRun(label=run.label, rt=run.rt, signal=sig_plot, results_df=run.results_df, color=run.color)
                _run_plot_peaks = run  # picos ainda em unidades originais → ajustar y
            else:
                _run_plot = run
                _run_plot_peaks = run

            self._add_chromatogram_trace(fig, _run_plot, show_in_legend=True)
            self._add_peak_markers(
                fig,
                _run_plot_peaks,
                show_in_legend=True,
                normalize_factor=(np.max(np.abs(run.signal)) if normalize else 1.0),
            )

        y_title = "Intensidade Normalizada" if normalize else "Intensidade (u.a.)"
        title_str = title or ("Sobreposição de Cromatogramas" + (" (Normalizado)" if normalize else ""))
        self._apply_common_layout(fig, title_str)
        fig.update_layout(yaxis=dict(title=y_title, showgrid=True, gridcolor="#ececec", zeroline=False))

        _fname = filename or self._default_html_name("overlay", [r.label for r in runs])
        self._save_and_show(fig, output_dir, _fname, show)
        return fig
    ...
    def plot_stacked(
        ...
        *,
        ...
        """Empilha cromatogramas verticalmente com offset. offset=None calcula automaticamente."""
        runs = self._select_runs(labels)
        if not runs:
            raise ValueError("[GCVisualizer] Nenhuma corrida selecionada para plot_stacked.")

        # Calcular offset automático se não fornecido
        if offset is None:
            max_signal = max(np.max(np.abs(r.signal)) for r in runs if len(r.signal) > 0)
            offset = max_signal * offset_factor

        fig = go.Figure()

        for i, run in enumerate(runs):
            shift = i * offset
            shifted_signal = run.signal + shift

            # Traço da linha (com shift)
            fig.add_trace(
                go.Scatter(
                    x=run.rt_min,
                    y=shifted_signal,
                    mode="lines",
                    name=run.label,
                    line=dict(color=run.color, width=self.line_width),
                    legendgroup=f"line__{run.label}",  # grupo próprio → independente dos picos
                    hovertemplate=(f"<b>{run.label}</b><br>" "RT: %{x:.3f} min<br>" "Int: %{customdata:.0f} u.a.<extra></extra>"),
                    customdata=run.signal,  # intensidade real (sem shift) no hover
                )
            )

            # Marcadores de pico (com shift)
            self._add_peak_markers(
                fig,
                run,
                show_in_legend=True,
            ...
            # Anotação de label na extremidade direita da curva
            fig.add_annotation(
                x=run.rt_min[-1],
                y=float(shifted_signal[-1]),
                text=f"<b>{run.label}</b>",
                showarrow=False,
                xanchor="left",
                xshift=6,
                font=dict(size=10, color=run.color),
            )

        title_str = title or "Cromatogramas Empilhados"
        self._apply_common_layout(fig, title_str)
        fig.update_layout(
            yaxis=dict(
                title="Intensidade (u.a.) + offset",
                showticklabels=False,  # valores de Y não têm significado absoluto
                showgrid=False,
                zeroline=False,
            )
        )

        _fname = filename or self._default_html_name("stacked", [r.label for r in runs])
        self._save_and_show(fig, output_dir, _fname, show)
        return fig
    ...
    def plot_all(
        self,
        *,
        normalize_overlay: bool = False,
        output_dir: Optional[str | Path] = None,
        show: Optional[bool] = None,
    ) -> dict[str, go.Figure]:
        """
        Conveniência: gera os três tipos de plot para todas as corridas
        adicionadas e salva cada um como HTML separado.

        Retorna
        -------
        dict com chaves "singles", "overlay", "stacked".
        """
        figs: dict[str, go.Figure] = {}
        singles = {}
        for run in self._runs:
            fname = self._default_html_name("single", [run.label])
            singles[run.label] = self.plot_single(run.label, output_dir=output_dir, filename=fname, show=show)
        figs["singles"] = singles  # type: ignore[assignment]

        figs["overlay"] = self.plot_overlay(
            normalize=normalize_overlay,
            output_dir=output_dir,
            filename=self._default_html_name("overlay", [r.label for r in self._runs]),
            show=show,
        )
        figs["stacked"] = self.plot_stacked(
            output_dir=output_dir,
            filename=self._default_html_name("stacked", [r.label for r in self._runs]),
            show=show,
        )
        return figs
    ...
    def _get_run(self, label: str) -> _VisRun:
        for r in self._runs:
            if r.label == label:
                return r
        available = [r.label for r in self._runs]
        raise KeyError(f"[GCVisualizer] Run {label!r} não encontrada. " f"Disponíveis: {available}")

    def _select_runs(self, labels: Optional[Sequence[str]]) -> list[_VisRun]:
        if labels is None:
            return list(self._runs)
        return [self._get_run(lbl) for lbl in labels]

    def _add_chromatogram_trace(
        self,
        fig: go.Figure,
        run: _VisRun,
        show_in_legend: bool = True,
    ) -> None:
        """Adiciona a curva do cromatograma ao figura."""
        fig.add_trace(
            go.Scatter(
                x=run.rt_min,
                y=run.signal,
                ...
                hovertemplate=("<b>" + run.label + "</b><br>" "RT: %{x:.3f} min<br>" "Int: %{y:.0f} u.a.<extra></extra>"),
            )
        )
    ...
    def _add_peak_markers(
        ...
        """
        Adiciona marcadores de pico para uma corrida.

        Parâmetros
        ----------
        y_shift : float
            Deslocamento vertical (para plot empilhado).
        normalize_factor : float
            Divisor para normalizar as alturas dos picos (para plot_overlay
            com normalize=True).
        """
        if not run.peak_rows:
            return

        xs, ys, hovers = [], [], []
        for i, row in enumerate(run.peak_rows, start=1):
            rt_s = row.get("marker_rt", row.get("rt", np.nan))
            ht = row.get("marker_height", row.get("height", np.nan))
            try:
                rt_val = float(rt_s)
                ht_val = float(ht)
            except (TypeError, ValueError):
                continue
            if not (np.isfinite(rt_val) and np.isfinite(ht_val)):
                continue

            xs.append(rt_val / 60.0)
            ys.append(ht_val / normalize_factor + y_shift)
            hovers.append(self._peak_hover_text(row, i))
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=ys,
                mode="markers",
                name=f"Picos — {run.label}",
                marker=dict(
                    color=run.color,
                    size=self.marker_size,
                    symbol="diamond",
                    line=dict(color="white", width=1.2),  # borda branca p/ contraste
                ),
                # legendgroup próprio → pode ser ligado/desligado independentemente da curva
                legendgroup=f"peaks__{run.label}",
                showlegend=show_in_legend,
    ...
    def _save_and_show(
        ...
        """Salva a figura como HTML e opcionalmente exibe no navegador."""
        _dir = output_dir if output_dir is not None else self.default_output_dir
        out_path = self._resolve_output(_dir, filename)
        ...
        print(f"[GCVisualizer] Salvo: {out_path}")

        _show = show if show is not None else self.show_on_plot
        if _show:
            fig.show(config=self._PLOTLY_CONFIG)
    ...
    def __repr__(self) -> str:
        runs_str = ", ".join(f"{r.label!r}({r.n_peaks}pk)" for r in self._runs) or "—"
        return f"GCVisualizer(runs=[{runs_str}], show={self.show_on_plot})"


if __name__ == "__main__":
    """
    exemplo_gc.py — Uso completo do pipeline GC com dados T1 e T2.

    Fluxo:
      1. Cria e salva um ProcessingMethod customizado
      2. Analisa T1 individualmente → relatório PDF + visualização
      3. Analisa T1 + T2 em lote → relatório comparativo PDF + visualizações (single, overlay, stacked)
      4. Extras: métricas estendidas, métricas globais, fingerprinting, alinhamento RRT, audit trail
    """

    import sys
    from pathlib import Path

    # ── Adicione o caminho do GC.py se necessário ────────────────────────────────
    # sys.path.insert(0, r"C:\caminho\para\pasta\com\GC.py")
    ...
    # ══════════════════════════════════════════════════════════════════════════════
    CDF_T1 = Path(r"C:\Users\Testes\cdf\T1.CDF")
    CDF_T2 = Path(r"C:\Users\Testes\cdf\T2.CDF")

    OUTPUT_DIR = Path(r"C:\Users\Testes\resultados")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    # ══════════════════════════════════════════════════════════════════════════════
    # 1. CRIAR E SALVAR O MÉTODO
    # ══════════════════════════════════════════════════════════════════════════════
    print("\n" + "=" * 60)
    print("ETAPA 1 — ProcessingMethod")
    print("=" * 60)

    metodo = ProcessingMethod(
        name="Metodo_Testes_v1",
        version="1.0",
        description="Método para análise dos cromatogramas T1 e T2",
        created_by="BDanielS",
        ...
        noise_percentile=20,
        snr_threshold=3.0,
        min_width_seconds=1.0,
        min_distance_seconds=2.0,
        # Solvent front — ignora picos antes de 60 s
        solvent_rt_cutoff_s=60.0,
        solvent_area_factor=5.0,
        t_start_integration=0,  # descarta a região do solvente
        # Sobreposição / deconvolução
        rs_deconv_threshold=1.2,
        valley_pct_independent=85.0,
        valley_pct_dropline=50.0,
        # Métricas estendidas
        dead_time_s=0.0,  # defina se souber o tempo morto (ex.: 30.0)
        # CQI — pesos e referências
        cqi_weight_n=1.0,
        cqi_weight_rs=1.0,
        cqi_weight_tf=1.0,
        ...
        cqi_rs_ref=1.5,
        cqi_snr_ref=10.0,
        # Suavização Savitzky-Golay (0 = automático)
        sg_window_length=0,
        sg_polyorder=4,
        # Slope-to-slope para delimitar bases dos picos
        slope_threshold_factor=0.10,
        # Número esperado de picos (opcional — ativa alerta de QC)
        expected_peaks_count=None,
    )

    caminho_metodo = OUTPUT_DIR / "metodo_testes_v1.json"
    metodo.save(caminho_metodo)
    print(f"Método salvo em: {caminho_metodo}")

    # Verificação de round-trip
    metodo_carregado = ProcessingMethod.load(caminho_metodo)
    print(f"Método recarregado: {metodo_carregado}")

    # ══════════════════════════════════════════════════════════════════════════════
    # 2. ANÁLISE INDIVIDUAL — T1
    # ══════════════════════════════════════════════════════════════════════════════
    print("\n" + "=" * 60)
    print("ETAPA 2 — Análise individual T1")
    print("=" * 60)

    analisador = GCAnalyzer(method=metodo, echo_audit=True)

    # 2a. Leitura
    rt1, raw1 = analisador.read_cdf(str(CDF_T1))
    print(f"T1: {len(rt1)} pontos | RT=[{rt1[0]:.1f}–{rt1[-1]:.1f}] s")

    # 2b. Baseline
    corrected1, baseline1 = analisador.remove_baseline(rt1, raw1)

    # 2c. Integração completa (find_peaks + integração adaptativa + remoção solvente)
    df1 = analisador.integrate(rt1, corrected1)

    # 2d. Métricas USP/EP (N, TF, As, W½, Wbase, SNR, área %)
    df1 = analisador.compute_usp_metrics(rt1, corrected1, df1)

    # 2e. Métricas estendidas (Rs USP/EP, k', α, SQS, CQI)
    df1 = analisador.compute_extended_metrics(rt1, corrected1, df1)

    # 2f. Métricas globais da corrida
    metricas_globais_t1 = analisador.compute_global_metrics(rt1, raw1, corrected1, baseline1, df1)

    print(f"\nT1 — {len(df1)} picos detectados")
    print(f"Métricas globais T1: {metricas_globais_t1}")
    print(df1[["rt", "area", "area_pct", "snr", "N_plates_ep", "tailing_factor_usp", "CQI"]].to_string())

    # 2g. Exportar resultado e audit trail
    df1.to_csv(OUTPUT_DIR / "T1_picos.csv", index=False)
    analisador.export_audit(
        path_json=str(OUTPUT_DIR / "T1_audit.json"),
        path_csv=str(OUTPUT_DIR / "T1_audit.csv"),
    )
    print("\nAudit T1:", analisador.audit.summary())

    # 2h. Plot interativo embutido (GCAnalyzer.plot_results)
    print("\nGerando plot interativo T1 (GCAnalyzer)...")
    analisador.plot_results(rt1, raw1, baseline1, corrected1, df1)

    # 2i. Relatório PDF — T1 apenas
    print("\nGerando relatório PDF T1...")
    relatorio_t1 = GCReport(
        analyzer=analisador,
        title="Relatório Cromatográfico — T1",
        analyst="name",
        instrument="GC",
        lab="lab",
        sample_info="Amostra T1",
    )
    relatorio_t1.add_run(rt1, raw1, corrected1, baseline1, df1, label="T1")
    pdf_t1 = relatorio_t1.build(str(OUTPUT_DIR / "relatorio_T1.pdf"))
    print(f"PDF gerado: {pdf_t1}")

    # 2j. Visualização via GCVisualizer — T1 apenas
    print("\nGerando visualização GCVisualizer T1...")
    viz_t1 = GCVisualizer(default_output_dir=OUTPUT_DIR, show_on_plot=True)
    viz_t1.add_run("T1", rt1, corrected1, df1)
    viz_t1.plot_single("T1", title="Cromatograma T1")

    # ══════════════════════════════════════════════════════════════════════════════
    # 3. ANÁLISE EM LOTE — T1 + T2
    # ══════════════════════════════════════════════════════════════════════════════
    print("\n" + "=" * 60)
    print("ETAPA 3 — Lote T1 + T2")
    print("=" * 60)

    # Novo analisador para o lote (audit limpo)
    analisador_lote = GCAnalyzer(method=metodo_carregado, echo_audit=False)

    batch_results, df_align_stats = analisador_lote.process_batch(
        cdf_files=[CDF_T1, CDF_T2],
        compute_usp=True,
        align=True,  # alinhamento RRT automático
    )

    for r in batch_results:
        print(r)
        if r.ok:
            # Métricas estendidas por corrida
            rt_tmp, raw_tmp = analisador_lote.read_cdf(r.cdf_path)
            corr_tmp, bl_tmp = analisador_lote.remove_baseline(rt_tmp, raw_tmp)
            r.results_df = analisador_lote.compute_extended_metrics(rt_tmp, corr_tmp, r.results_df)
            gm = analisador_lote.compute_global_metrics(rt_tmp, raw_tmp, corr_tmp, bl_tmp, r.results_df)
            print(
                f"  Métricas globais {r.run_id}: OQS={gm['overall_quality_score']:.3f} | "
                f"SNR_global={gm['global_snr']} | n_picos={gm['n_peaks']}"
            )
            r.results_df.to_csv(OUTPUT_DIR / f"{r.run_id}_picos.csv", index=False)

    # Estatísticas de alinhamento
    if df_align_stats is not None:
        print("\nAlinhamento RRT (bins):")
        print(df_align_stats.to_string())
        df_align_stats.to_csv(OUTPUT_DIR / "alinhamento_rrt.csv", index=False)

    # 3a. Fingerprinting — comparação ponto-a-ponto T1 vs T2
    print("\nFingerprinting T1 vs T2...")
    r_t1 = next(r for r in batch_results if r.run_id == "T1")
    r_t2 = next(r for r in batch_results if r.run_id == "T2")

    if r_t1.ok and r_t2.ok:
        rt1b, raw1b = analisador_lote.read_cdf(r_t1.cdf_path)
        corr1b, _ = analisador_lote.remove_baseline(rt1b, raw1b)
        rt2b, raw2b = analisador_lote.read_cdf(r_t2.cdf_path)
        corr2b, _ = analisador_lote.remove_baseline(rt2b, raw2b)

        fp = GCAnalyzer.compare_chromatograms(rt1b, corr1b, rt2b, corr2b)
        print(
            f"Fingerprinting: Pearson r={fp['pearson_r']:.4f} | Cosine={fp['cosine_similarity']:.4f} | Veredicto={fp['verdict']}"
        )

    # 3b. Audit trail do lote
    analisador_lote.export_audit(
        path_json=str(OUTPUT_DIR / "lote_audit.json"),
        path_csv=str(OUTPUT_DIR / "lote_audit.csv"),
    )

    # 3c. Relatório PDF comparativo — T1 + T2
    print("\nGerando relatório PDF comparativo T1 + T2...")
    relatorio_lote = GCReport(
        analyzer=analisador_lote,
        title="Relatório Cromatográfico — T1 vs T2",
        analyst="name",
        instrument="GC",
        lab="lab",
        sample_info="Comparação T1 e T2",
    )

    for r in batch_results:
        if r.ok:
            rt_l, raw_l = analisador_lote.read_cdf(r.cdf_path)
            corr_l, bl_l = analisador_lote.remove_baseline(rt_l, raw_l)
            relatorio_lote.add_run(rt_l, raw_l, corr_l, bl_l, r.results_df, label=r.run_id)

    pdf_lote = relatorio_lote.build(str(OUTPUT_DIR / "relatorio_T1_T2.pdf"))
    print(f"PDF comparativo gerado: {pdf_lote}")

    # 3d. GCVisualizer — T1 + T2 (single, overlay, stacked)
    print("\nGerando visualizações GCVisualizer T1 + T2...")
    viz_lote = GCVisualizer(default_output_dir=OUTPUT_DIR, show_on_plot=True)

    for r in batch_results:
        if r.ok:
            rt_v, raw_v = analisador_lote.read_cdf(r.cdf_path)
            corr_v, _ = analisador_lote.remove_baseline(rt_v, raw_v)
            viz_lote.add_run(r.run_id, rt_v, corr_v, r.results_df)

    # Plot individual de cada corrida
    for label in viz_lote.runs():
        viz_lote.plot_single(label, title=f"Cromatograma {label}")

    # Sobreposição normalizada
    viz_lote.plot_overlay(normalize=True, title="Sobreposição T1 + T2 (normalizado)")

    # Empilhado
    viz_lote.plot_stacked(title="Cromatogramas Empilhados T1 + T2")

    # ── Ou gera tudo de uma vez ──
    # viz_lote.plot_all(normalize_overlay=True)

    print("\n" + "=" * 60)
    print("CONCLUÍDO — arquivos salvos em:", OUTPUT_DIR)
    print("=" * 60)
